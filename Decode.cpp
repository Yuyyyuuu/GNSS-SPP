...
函数功能：对Buff中的数据解码存储，在遇到观测值类型数据后返回1在主函数内定位解算
********************************************************************************
*/
int DecodeNovOem7Dat(unsigned char Buff[], int& Len, EPOCHOBS* obs, GPSEPHREC geph[], GPSEPHREC beph[],POSRES* pres)
{
	...
	{	
		...
		{
			i = i + len+4;
			continue;// 这个不同于完整性检测，完整性不够的话就需要Buff新读入数据，而crc不对则可以继续在Buff里遍历到新的message
		}
...
int decode_rangeb_oem7(unsigned char* buff, EPOCHOBS* obs)
{
	...
	for (int i = 0; i < obs_num; i++)
	{
		...
		// 解码其他数据
		short half = (U4(buff + OEM7HLEN + 44 + 44 * i) >> 28) & 0x1;
